...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void addCarToStorage(Car car) {
            if (car == null) throw new IllegalArgumentException("kar is null");
            if (car.getPrice() < 0 ) throw new IllegalArgumentException("kar is null");
    ...
    public void sellCar(Car car) throws RequestedCarNotFoundException {
            if (car == null) throw new IllegalArgumentException("kar is null");
            if (!CarShopStorage.getInstancce().getCars().remove(car.getType(), car)){
                throw new RequestedCarNotFoundException("neni");
            }
